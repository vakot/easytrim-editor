...
async function bootstrapNativeDiagnostics(): Promise<DiagnosticsBootstrap | null> {
  if (!isTauriRuntime()) return null;
  return invoke<DiagnosticsBootstrap>("diagnostics_bootstrap");
}

async function completeDiagnosticsSession(): Promise<void> {
  if (!isTauriRuntime()) return;
  await invoke("complete_diagnostics_session");
}

async function persistDiagnosticEvent(event: DiagnosticEventInput): Promise<void> {
  if (!isTauriRuntime()) return;
  await invoke("record_diagnostic_event", { event });
}

async function listPersistedDiagnosticSessions(): Promise<DiagnosticSessionSummary[]> {
  if (!isTauriRuntime()) return [];
...
async function readPersistedDiagnosticSessionEvents(sessionId: string): Promise<DiagnosticEvent[]> {
  if (!isTauriRuntime()) return [];
...
async function recordUiHeartbeat(): Promise<void> {
  if (!isTauriRuntime()) return;
  await invoke("record_ui_heartbeat");
}

async function revealDiagnosticLogs(): Promise<void> {
  if (!isTauriRuntime()) return;
  await invoke("reveal_diagnostic_logs");
}

async function revealDiagnosticReport(recovery: StartupRecovery): Promise<void> {
  if (!isTauriRuntime()) return;
