...
describe("media IPC adapter", () => {
  ...
  it("parses direct and proxy preview descriptors through narrow commands", async () => {
    mocks.invoke
      .mockResolvedValueOnce({
        sourceId: "source-3",
        url: "http://framelatch-media.localhost/source-3?variant=source",
        kind: "source",
      })
      .mockResolvedValueOnce({
        sourceId: "source-3",
        url: "http://framelatch-media.localhost/source-3?variant=proxy",
        kind: "proxy",
      });
  ...
  it("rejects an unknown preview kind at the IPC boundary", async () => {
    mocks.invoke.mockResolvedValue({
      sourceId: "source-3",
      url: "http://framelatch-media.localhost/source-3",
      kind: "filesystem",
    });
    ...
    mocks.invoke.mockResolvedValue([
      {
        ...
        streamIndex: 2,
        width: 1280,
        url: "http://framelatch-media.localhost/source-3?variant=waveform&stream=2&width=1280",
      },
      {
        status: "failed",
        sourceId: "source-3",
