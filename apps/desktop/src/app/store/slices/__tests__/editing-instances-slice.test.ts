...
function attempt(id: string, snapshot = baseSnapshot) {
  return createExportAttempt({
    capturedAt: 10,
    id,
    output: { displayName: `${id}.mp4`, displayPath: `C:/Exports/${id}.mp4`, outputId: `output-${id}` },
    request: {
      audioTracks: [],
      mergeAudio: false,
...
describe("editing instances slice", () => {
  it("keeps imported and duplicated instances as stable independent identities", () => {
    ...
    const state = editingInstancesReducer(initialEditingInstancesState, editingInstancesAdded([first, duplicate]));

    expect(state.ids).toEqual(["instance-1", "instance-2"]);
    expect(state.entities["instance-1"]?.snapshot.trim).toEqual({ kind: "full-source" });
    expect(state.entities["instance-2"]?.snapshot.trim).toEqual({ endMicros: 2_000_000, startMicros: 500_000 });

    const explicitDuplicate = { ...duplicate, id: "instance-3", origin: "duplicate" as const };
    const duplicatedState = editingInstancesReducer(state, editingInstanceDuplicated(explicitDuplicate));
  ...
  it("captures an attempt snapshot and ignores stale terminal callbacks", () => {
    ...
    let state = editingInstancesReducer(initialEditingInstancesState, editingInstancesAdded([instance("instance-1")]));
    state = editingInstancesReducer(state, activeEditingInstanceChanged("instance-1"));
    state = editingInstancesReducer(state, editingInstanceExportAttemptQueued({ id: "instance-1", attempt: queuedAttempt }));
    state = editingInstancesReducer(state, editingInstanceExportStarted({ attemptId: "attempt-1", id: "instance-1", startedAt: 20 }));
    state = editingInstancesReducer(state, editingInstanceExportProgressReceived({
      attemptId: "attempt-1",
      id: "instance-1",
      metrics: { currentFrame: 4, progressPercent: 40 },
      progress: { elapsedMicros: 400_000, frame: 4, operationId: "operation-1", phase: "running", speed: "1x", totalSize: 42 },
    }));
    ...
    state = editingInstancesReducer(state, editingInstanceSnapshotUpdated({ id: "instance-1", snapshot: editedSnapshot }));
    state = editingInstancesReducer(state, editingInstanceExportCompleted({
      attemptId: "attempt-1",
      durationMs: 100,
      id: "instance-1",
      result: { displayName: "attempt-1.mp4", displayPath: "C:/Exports/attempt-1.mp4", operationId: "operation-1" },
    }));
    state = editingInstancesReducer(state, editingInstanceExportFailed({
      attemptId: "attempt-1",
      durationMs: 200,
  ...
  it("clears only terminal history and keeps the editing instance", () => {
    ...
    let state = editingInstancesReducer(initialEditingInstancesState, editingInstancesAdded([instance("instance-1")]));
    state = editingInstancesReducer(state, editingInstanceExportAttemptQueued({ id: "instance-1", attempt: queued }));
    ...
    state = editingInstancesReducer(state, editingInstanceExportStarted({ attemptId: "queued", id: "instance-1", startedAt: 20 }));
    state = editingInstancesReducer(state, editingInstanceExportCompleted({
      attemptId: "queued",
      durationMs: 100,
      id: "instance-1",
      result: { displayName: "completed.mp4", displayPath: "C:/Exports/completed.mp4", operationId: "operation-2" },
    }));
  ...
  it("propagates source availability across instances sharing one source path", () => {
    const sameSource = instance("instance-2", { ...baseSnapshot, source: { ...firstSource, displayName: "copy.mp4" } });
    let state = editingInstancesReducer(initialEditingInstancesState, editingInstancesAdded([instance("instance-1"), sameSource, instance("instance-3", { ...baseSnapshot, source: secondSource })]));
    state = editingInstancesReducer(state, editingInstancesSourceAvailabilityChanged({ availability: "deleted", sourcePath: firstSource.sourcePath }));
  ...
  it("closes one instance without deleting another instance or its history", () => {
    ...
    let state = editingInstancesReducer(initialEditingInstancesState, editingInstancesAdded([first, instance("instance-2")]));
    state = editingInstancesReducer(state, activeEditingInstanceChanged("instance-1"));
