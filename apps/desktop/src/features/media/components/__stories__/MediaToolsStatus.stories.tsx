...
import { AppTitleBarCommandCenter } from "@/app/layout/components/AppTitleBarCommandCenter";
...
const meta = {
  component: AppTitleBarCommandCenter,
  args: { state: "checking" },
  argTypes: {
    state: {
      control: "select",
  ...
  render: ({ state }) => <MediaToolsStatusStory state={state} />,
  tags: ["autodocs"],
...
function MediaToolsStatusStory({ state }: MediaToolsStatusStoryArgs) {
      ...
      <CommandPaletteProvider>
        <div className="grid justify-items-start gap-3">
          <div className="flex h-10 items-center rounded-lg border bg-background px-1">
            <AppTitleBarCommandCenter key={animationRun} />
          </div>
