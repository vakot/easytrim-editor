...
fn preview_url(source_id: &str, kind: PreviewKind) -> String {
    format!(
        "http://framelatch-media.localhost/{source_id}?variant={}",
...
fn preview_url(source_id: &str, kind: PreviewKind) -> String {
    format!(
        "framelatch-media://localhost/{source_id}?variant={}",
        preview_kind_name(kind)
    )
}
...
fn waveform_url(source_id: &str, stream_index: u32, width: u32) -> String {
    format!(
        "http://framelatch-media.localhost/{source_id}?variant=waveform&stream={stream_index}&width={width}"
    )
}

#[cfg(any(target_os = "windows", target_os = "android"))]
fn audio_preview_url(source_id: &str, stream_index: u32) -> String {
    format!("http://framelatch-media.localhost/{source_id}?variant=audio&stream={stream_index}")
}

#[cfg(not(any(target_os = "windows", target_os = "android")))]
fn audio_preview_url(source_id: &str, stream_index: u32) -> String {
    format!("framelatch-media://localhost/{source_id}?variant=audio&stream={stream_index}")
}

#[cfg(not(any(target_os = "windows", target_os = "android")))]
fn waveform_url(source_id: &str, stream_index: u32, width: u32) -> String {
    format!(
        "framelatch-media://localhost/{source_id}?variant=waveform&stream={stream_index}&width={width}"
    )
}
