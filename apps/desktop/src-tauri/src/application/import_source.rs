...
pub fn import_webcam_source(state: &AppState, path: PathBuf) -> Result<SourceSelection, AppError> {
    let generation = state.begin_webcam_replacement()?;
    let source = validate_source(&path)?;
    let source_id = state.complete_webcam_replacement(generation, source)?;
    let active_source = state.resolve_source(&source_id)?;
    let display_name = active_source
        .path
        .file_name()
        .map(|name| name.to_string_lossy().into_owned())
        .ok_or_else(|| AppError::internal("The webcam source has no usable file name."))?;
