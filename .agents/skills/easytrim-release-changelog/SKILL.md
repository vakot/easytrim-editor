...
4. If no applicable stable tag exists, do not invent a baseline or silently use an arbitrary commit.
   Report that the release range cannot be determined and ask for the intended baseline.
...
- tests, documentation, configuration, and user-facing strings that confirm visible behavior.
...
7. Review the complete resulting `CHANGELOG.md` against the release range and confirm every entry
   has direct evidence in the final source diff or clearly supporting repository evidence.
